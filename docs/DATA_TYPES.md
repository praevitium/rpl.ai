...
| Op    | R | Z | Q | B | C | N | Sy | L | V | M | T | U | S | P | Notes |
|-------|---|---|---|---|---|---|----|---|---|---|---|---|---|---|-------|
| FLOOR | ✓ | ✓ | ✓ | ✓ | ✗ | ✓ | ✓  | ✓ | ✓ | ✓ | ✓ | ✓ | ✗ | ✗ | Unit preserved (`1.5_m FLOOR` → `1_m`). A quoted number folds (`` `3.7` FLOOR `` → `3.`; CEIL / IP / FP too). B is a no-op (BinInt always integer). Complex ✗ — no total order. T transparent (`:x:Real(7.2) FLOOR` → `:x:7`). Q→Z collapse: `FLOOR Rational(7,2)` → `Integer(3)`, `FLOOR Rational(-7,2)` → `Integer(-4)` (round toward -∞); APPROX-mode → Real(3). Tagged-of-List composition (`_withTaggedUnary` ∘ `_withListUnary`): `:lbl:{Real(7.2) Real(-1.5)} FLOOR` → `:lbl:{Real(7) Real(-2)}`, with recursion through nested Lists. |
