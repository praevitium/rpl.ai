...
| UTPF      | ✓ | ✓ | ✗ | ✗ | ✓ | ✓  | ✗ | ✗ | ✗ | ✓ | ✗ | Upper-tail F CDF. Arity 3 — UTPF(ν₁, ν₂, x). No simplify-time fold (needs incomplete beta). Sy round-trips; null fold. Z `✓`: same `asReal` helper (shared with UTPC/UTPT) accepts Integer — `UTPF(Integer(2), Integer(2), Integer(1))` → `Real(0.5)`. Bare handler (3-arg; no `_withListBinary` shape): L `✗` / V `✗` / M `✗` — a List/Vector/Matrix in any operand slot reaches `asReal` → Bad argument type (so the variate position rejects rather than distributing, unlike UTPC/UTPT). L/V/M rejection pinned session310 (variate + ν positions). Q `✗` / C `✗` / S `✗`: same `asReal` helper rejects all three (first String operand throws). |
